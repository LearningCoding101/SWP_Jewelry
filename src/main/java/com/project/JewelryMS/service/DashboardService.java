...
//       categoryResponseList.sort(Comparator.comparing(CategoryResponse::getCategory_Total).reversed());
    ...
    public List<StaffSalesResponse> getSalesMadeByStaff(LocalDate date) {
        LocalDateTime startDateTime = date.atStartOfDay();
        LocalDateTime endDateTime = date.atTime(LocalTime.MAX);
        ...
        for (PurchaseOrder order : orders) {
            ...
            if (staffAccount != null) {
                int staffId = staffAccount.getStaffID();
                int orderQuantity = order.getOrderDetails().stream()
                        .mapToInt(OrderDetail::getQuantity)
                        .sum();
                salesByStaff.merge(staffId, orderQuantity, Integer::sum);
            }
        }
        ...
        for (Map.Entry<Integer, Integer> entry : salesByStaff.entrySet()) {
            StaffAccount staff = staffAccountRepository.findById(entry.getKey()).orElse(null);
            if (staff != null) {
                responseList.add(new StaffSalesResponse(staff.getAccount().getUsername(), entry.getValue()));
            }
        }
